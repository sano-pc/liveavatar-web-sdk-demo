...
Root Directoryが正しく設定されているか確認してください。

### Output Directoryのパスが二重になる

Root Directoryが `apps/demo` の場合、Output Directoryは `.next` のみにしてください（`apps/demo/.next` ではない）。

### APIルートがエラーを返す

1. 環境変数がすべて設定されているか確認
2. `LIVEAVATAR_CONTEXT_ID` に先頭スペースがないか確認
3. Vercel Dashboardでビルドログを確認

### デプロイ後に認証画面が表示される

Deployment Protectionが有効になっています。設定方法は [vercel-deployment-protection.md](./vercel-deployment-protection.md) を参照してください。

## 関連ドキュメント

- [Vercel Deployment Protection 設定ガイド](./vercel-deployment-protection.md)
